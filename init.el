...
(use-package awesome-tab
  :load-path "~/latex-emacs30/elisp/awesome-tab"
  :config
  (awesome-tab-mode t))
...
(add-to-list 'load-path "~/latex-emacs30/manual-packages/electric-spacing")
...
;;*** SLIME
;; mode for common lisp. The config below is from https://www.emacswiki.org/emacs/SlimeMode

(use-package slime
    :ensure t)
    
(defun slime-repl-mode-end-of-defun () (forward-sexp) t)
...
;; Emacs’s elisp documentaiton states that “Sequences consisting of ‘C-c’ and a letter (either upper or lower case) are reserved for users; 
;; they are the only sequences reserved for users, so do not block them.” Unfortunately, slime binds ‘C-c x’ to slime-export-symbol-at-point, 
;; shadowing any keybindings the user might have established via global-set-key.
...
;; ** U
;; *** undo-tree
;; See https://www.dr-qubit.org/undo-tree/undo-tree.txt
(use-package undo-tree
    :init
    (global-undo-tree-mode)
    :custom
    (undo-tree-history-directory-alist '(("." . "~/latex-emacs30/undo")))
    )
